/*  
 * This is a modified version of JHeatChart 0.6 by TomCastle
 * This version instead of using different bightness opf the same color to map values, uses different color
 * From low to high values you'll get: black->blue->green->yellow->red
 * As requested by the original license, this is released under GPL license
 * The follow is the original comment:
...
public class HeatChart {
	...
	private Color getCellColour(double data, double min, double max) {		
		...
		//We use a multi-color representation, that's the difference of this software
		return getHeat(percentPosition);
	}
	
	/**Calculate the heat colour as we intend it*/
	private Color getHeat(double percent){
		//let's say increasing heat we have:
		//black->blue->green->yellow->red
		
		int red=0,blue=0,green=0;
		
		if(percent>0.5){
			red=(int) (Math.round(255.0*percent*2.0)-255);
		}
		
		if(percent<0.5){
			blue=(int) (255- 255.0*Math.abs(percent-0.25));
		}
		
		if(percent>0.25 && percent<0.5){
			green=(int) (percent*255.*4.-255.0);
		}
		
		if(percent>0.5){
			green=(int) (511.0-percent*255.*2.);
		}
		if(red<0 || green<0 || blue<0 || red>255 || green>255 || blue > 255){
			System.err.println("valori "+red+","+green+","+blue+" per "+percent);
